...
// ========================================================================
// Function for starting our input loop.
void zgui::poll_input(HWND hwnd, int w, int h) {
    ...
    p_mouse_pos.x = std::clamp(static_cast<int>(p_mouse_pos.x), 0, w);
    p_mouse_pos.y = std::clamp(static_cast<int>(p_mouse_pos.y), 0, h);

    window_ctx.position.x = static_cast<float>(std::clamp(static_cast<int>(window_ctx.position.x), 0, static_cast<int>(static_cast<float>(w) - window_ctx.size.x)));
    window_ctx.position.y = static_cast<float>(std::clamp(static_cast<int>(window_ctx.position.y), 0, static_cast<int>(static_cast<float>(h) - window_ctx.size.y)));
...
// ========================================================================
void zgui::color_picker(const char* id, color& item, int box_offset) noexcept
{
    std::vector<std::string> id_split = utils::hash::split_str(id, '#');

    constexpr int control_height = 8;
    constexpr int control_width = 8;

    constexpr int slider_height = 10;
    constexpr int slider_width = 152;
    ...
    functions.get_text_size(window_ctx.fonts.top(), id_split[0].c_str(), text_wide, text_tall);

    window_ctx.render.emplace_back(zgui_control_render_t{ {draw_pos.x - 3 , draw_pos.y - 3}, zgui_render_type::zgui_text, global_colors.color_text, id_split[0], vec2{0, 0}, window_ctx.fonts.top() });

    draw_pos.x += box_offset;

    constexpr float x_offset = 75;
    ...
    const bool active = window_ctx.blocking == zgui::utils::hash::hash(id);
    ...
    {
        window_ctx.blocking = zgui::utils::hash::hash(id);
    }
    else if (active)
    {
        ...
        for (int i = 0; i < 990; i++)
        {
            ...
            window_ctx.render.emplace_back(zgui_control_render_t{ { xoffset + 110, yoffset, }, zgui_render_type::zgui_filled_rect,color { rainbow.r, rainbow.g, rainbow.b, global_colors.control_outline.a },"", { 5,5 } });
        ...
        for (int i = 0; i < 40; i++)
        {
            ...
            window_ctx.render.emplace_back(zgui_control_render_t{ { draw_pos.x + 20, (yoffset + 5) - 170, }, zgui_render_type::zgui_filled_rect,color { grey.r, grey.g, grey.b, global_colors.control_outline.a },"", { 10,4 } });
        ...
        if (utils::input::key_down(VK_LBUTTON) && utils::input::mouse_in_region(sliderxoffset, slideryoffset, slider_width + 5, slider_height) && window_ctx.blocking == zgui::utils::hash::hash(id)) {
            window_ctx.blocking = zgui::utils::hash::hash(id);
        ...
        window_ctx.render.emplace_back(zgui_control_render_t{ { sliderxoffset + 1 , slideryoffset + 1  }, zgui_render_type::zgui_filled_rect, color{item.r,item.g,item.b,item.a}, "", { static_cast<float>(dynamic_width), control_height - 2 } });
        window_ctx.render.emplace_back(zgui_control_render_t{ { sliderxoffset + 1 , slideryoffset + 1 }, zgui_render_type::zgui_filled_rect, global_colors.control_idle,"", { slider_width - 2, control_height - 2 } });
        window_ctx.render.emplace_back(zgui_control_render_t{ { sliderxoffset , slideryoffset  }, zgui_render_type::zgui_filled_rect, global_colors.control_outline,"", { slider_width, control_height } });
    }

    if (global_colors.control_outline.a >= 250)
        window_ctx.render.emplace_back(zgui_control_render_t{ { draw_pos.x, draw_pos.y }, zgui_render_type::zgui_filled_rect, color{item.r,item.g,item.b,item.a},"", { 2 * control_width, control_height } });
    else
        window_ctx.render.emplace_back(zgui_control_render_t{ { draw_pos.x, draw_pos.y }, zgui_render_type::zgui_filled_rect, color{item.r,item.g,item.b,global_colors.control_outline.a},"", { 2 * control_width, control_height } });

    window_ctx.render.emplace_back(zgui_control_render_t{ { draw_pos.x - 1, draw_pos.y - 1 }, zgui_render_type::zgui_rect, color{0,0,0,global_colors.control_outline.a},"", { 2 * control_width + 2, control_height + 2 } });

    utils::misc::push_cursor_pos(vec2{ cursor_pos.x + 14 + global_config.item_spacing, cursor_pos.y });
    utils::misc::push_cursor_pos(vec2{ cursor_pos.x, cursor_pos.y });

    const vec2 cursor_pos_temp = utils::misc::pop_cursor_pos();
    utils::misc::push_cursor_pos(vec2{ cursor_pos_temp.x, cursor_pos_temp.y + global_config.item_spacing });
}
// ========================================================================
void zgui::begin_groupbox(std::string_view title, const vec2 size) noexcept
{
    ...
    {
        int text_wide, text_tall;
        functions.get_text_size(window_ctx.fonts.top(), title.data(), text_wide, text_tall);
        functions.draw_text(draw_pos.x + size.x / 2 - text_wide / 2, draw_pos.y - 8, global_colors.color_text, window_ctx.fonts.top(), false, title.data());
    }
...
bool zgui::tab_button(const char* id, const vec2 size, bool& value) noexcept
{
    ...
    const bool active = window_ctx.blocking == zgui::utils::hash::hash(id);
    const bool hovered = utils::input::mouse_in_region(draw_pos.x, draw_pos.y, size.x, size.y);
    ...
    window_ctx.render.emplace_back(zgui_control_render_t{ {draw_pos.x + size.x / 2 - text_wide / 2, draw_pos.y + size.y / 2 - text_tall / 2 }, zgui_render_type::zgui_text, global_colors.color_text,  id_split[0]});

    if (value)
    {
        window_ctx.render.emplace_back(zgui_control_render_t{ { draw_pos.x, draw_pos.y + size.y - 2 }, zgui_render_type::zgui_filled_rect, global_colors.control_active_or_clicked, "", {size.x - 1, 1} });
    }


    window_ctx.render.emplace_back(zgui_control_render_t{ { draw_pos.x + 1, draw_pos.y + 1 }, zgui_render_type::zgui_filled_rect,hovered ? global_colors.window_border_fill : global_colors.control_idle, "",{size.x - 2, size.y - 2} });
    window_ctx.render.emplace_back(zgui_control_render_t{ { draw_pos.x, draw_pos.y }, zgui_render_type::zgui_filled_rect, global_colors.control_outline, "", size });
...
// ========================================================================
void zgui::menu_color_options() noexcept
{
    static const decltype(global_colors) original_menu_colors = global_colors;

    if (button("Reset Default Menu Colors", { 150 , 20 }))
        global_colors = original_menu_colors;

    color_picker("Combobox Selected Item Color", global_colors.color_combo_bg, 150);
    color_picker("Slider Color", global_colors.color_slider, 150);
    color_picker("Text Color", global_colors.color_text, 150);
    color_picker("Control Text Color", global_colors.color_text_dimmer, 150);
    color_picker("Control Idle Color", global_colors.control_idle, 150);
    color_picker("Active or Clicked Color", global_colors.control_active_or_clicked, 150);
    color_picker("Control Outline Color", global_colors.control_outline, 150);
    color_picker("Background Color", global_colors.window_background, 150);
    color_picker("Border Color", global_colors.window_border_color, 150);
    color_picker("Border Fill Color", global_colors.window_border_fill, 150);
    color_picker("Border Inner Fill Color", global_colors.window_border_inner_fill, 150);
}
// ========================================================================
decltype(global_colors)* zgui::get_menu_colors() noexcept
{
    return &global_colors;
}
// ========================================================================
void zgui::text_input(const char* id, std::string& value, int control_width, int control_height, const int max_length, const int flags) {
    std::vector<std::string> id_split = utils::hash::split_str(id, '#');
    ...
    if (hovered && utils::input::key_pressed(VK_LBUTTON) && !active) {
        ...
        if (utils::input::key_pressed(VK_ESCAPE) || utils::input::key_pressed(VK_RETURN) || (!hovered && utils::input::key_pressed(VK_LBUTTON))) {
            ...
            for (int i = 32; i <= 222; i++) {
                ...
                if (i == VK_SPACE && utils::input::key_pressed(VK_SPACE)) {
                    value += ' ';
                    continue;
                }
    ...
    window_ctx.render.emplace_back(zgui_control_render_t{ {draw_pos.x + 4, draw_pos.y + 4}, zgui_render_type::zgui_text, global_colors.color_text, flags & zgui_text_input_flags_password ? std::string(value.length(), '*').c_str() : value.c_str(), vec2{0, 0}, font });
    window_ctx.render.emplace_back(zgui_control_render_t{ {draw_pos.x + 1, draw_pos.y + 1}, zgui_render_type::zgui_filled_rect, active ? global_colors.control_active_or_clicked : global_colors.control_idle, "", {static_cast<float>(control_width) - 2, static_cast<float>(control_height) - 2}});
    window_ctx.render.emplace_back(zgui_control_render_t{ {draw_pos.x, draw_pos.y}, zgui_render_type::zgui_filled_rect, global_colors.control_outline, "", {static_cast<float>(control_width), static_cast<float>(control_height)} });
...
// ========================================================================
void zgui::combobox(const char* id, std::vector<std::string> items, int& value, const float control_width, const float control_height) {
    std::vector<std::string> id_split = utils::hash::split_str(id, '#');
...
// ========================================================================
void zgui::multi_combobox(const char* id, std::vector<multi_select_item_thread_safe>& items) {
    std::vector<std::string> id_split = utils::hash::split_str(id, '#');
